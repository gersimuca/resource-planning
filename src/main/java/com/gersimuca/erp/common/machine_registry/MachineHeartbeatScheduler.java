...
/**
 * Keeps this pod's machine-id lease alive by periodically proving it's still up, and triggers
 * a fresh allocation the moment it discovers that proof failed.
 *
 * <p>This is the piece that was missing from the original design: a lease that's never
 * refreshed looks identical, from {@link MachineRegistryStaleCleanupServiceImpl}'s point of
 * view, to a lease held by a pod that crashed - so without this scheduler, every pod's machine
 * id would be reclaimed and handed to someone else within the stale-cleanup cutoff, whether or
 * not the original pod was still alive and still using it.
 */
@Component
...
public class MachineHeartbeatScheduler {
     ...
     * cutoff, so a single missed tick (a slow GC pause, a momentary DB hiccup) doesn't by itself
     * risk the lease being reclaimed; several consecutive misses would have to happen first.
     */
    ...
    public void heartbeat() {
        ...
        final boolean stillOwned = registryService.updateHeartbeat(new MachineId(currentMachineId), instanceId);
        if (!stillOwned) {
